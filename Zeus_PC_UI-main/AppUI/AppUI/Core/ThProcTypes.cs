...
public record class ThProcTransportPatientInfo
{
  ...
  // 表示用ラベル：UIはこれを表示（HomeScreen.xaml の「転送ファイル」列）
  public string TransportTypeLabel => TransportType switch
  {
    CTDataTransportType.Xreal => "XREAL",
    CTDataTransportType.iPhone => "iPad",         // 表示だけ iPad に統一
    CTDataTransportType.XrealIPhone => "XREAL/iPad",
    _ => ""
  };
}
