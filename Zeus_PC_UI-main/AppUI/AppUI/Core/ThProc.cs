...
public partial class ThProc
{
    ...
    public static Task<ThProcCmdStatus> ExecuteAISegmentatorCommand(
        string studyUid, 
        PositionalCTType selectPosition, 
        out ThProcCmdOutput output,
        CancellationTokenSource cts, Action<ThProcCmdStatus>? onProgress = null, Action<ThProcCmdStatus>? onSucceeded = null, Action<ThProcCmdStatus>? onFailed = null) =>
        ExecuteAsyncCommand(new CsCmd { cmdID = (int)ExecCmdType.ExeAISegmentator, strParam1 = studyUid, intParam = (int)selectPosition }, out output, cts, onProgress, onSucceeded, onFailed);       // AIセグメント(Async)
