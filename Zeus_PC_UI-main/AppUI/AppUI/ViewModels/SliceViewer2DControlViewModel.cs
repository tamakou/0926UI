...
public partial class SliceViewer2DControlViewModel : ObservableObject, IDisposable
{
    ...
    public async Task Initialize(string studyUid, bool isImportData = false)
    {
        ...
        if (isImportData)
        {
            IsPositionSelectExec = false;

            await LoadVolume();

            PositionSelectMessage = MessageService.GetMessage(MessageCode.DlgMsgImportPositionSelect);
    ...
    [ObservableProperty]
    public bool isPositionSelectExec = false;

    [ObservableProperty]
    public bool isPositionSelect = false;

    [ObservableProperty]
    public string positionSelectMessage = "";

    private PositionalCTType SelectPositionType { get; set; } = PositionalCTType.Supine;
    ...
    private async Task InitializeAsync(PositionalCTType position)
    {
        IsPositionSelectExec = false;
        IsPositionSelect = false;
        ...
        await Processing3DImage(position);

        StatusMessage = complete3DMessage;
        IsConverting3D = false;
    }

    private async Task Processing3DImage(PositionalCTType position)
    {
        var cts = new CancellationTokenSource();
        ...
        {
            var cnt = 0;
            var poolingTask = ThProc.ExecuteAISegmentatorCommand(
                StudyUID,
                position,
    ...
    [RelayCommand]
    private void SupineButton(object param)
    {
        if (param is not System.Windows.Controls.Button btn) return;

        SelectPositionType = btn.Name switch
        {
            "SupineBtn" => PositionalCTType.Supine,
            "LeftDecubitusBtn" => PositionalCTType.LeftDecubitus,
            "RightDecubitusBtn" => PositionalCTType.RightDecubitus,
            _ => PositionalCTType.Supine,
        };
        var posotionStr = SelectPositionType switch
        {
            PositionalCTType.Supine => "仰臥位",
            PositionalCTType.LeftDecubitus => "左側臥位",
            PositionalCTType.RightDecubitus => "右側臥位",
            _ => "",
        };
        IsPositionSelectExec = true;
        var dlgMessage = MessageService.GetMessage(MessageCode.DlgMsgImportPositionExec);
        PositionSelectMessage = string.Format(dlgMessage, posotionStr);
    }
    [RelayCommand]
    private async Task SupineCorrect(object param)
    {
        if (param is not System.Windows.Controls.Button btn) return;

        var position = SelectPositionType;
        if (btn.Name == "Yes")
        {
            await InitializeAsync(position);
        }
        else
        {
            IsPositionSelectExec = false;
...
public class HeightReductionConverter : IValueConverter
{
    public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
    {
        var originalHeight = (double)value;
        var reduceBy = parameter != null ? double.Parse(parameter.ToString() ?? "0") : 0;
        var ret = originalHeight - reduceBy;
        return ret >= 0 ? ret : 0;
    }
