...
public:
	...
	virtual bool get_dll_called_log(void) {
			return false;
	}
protected:
...
public:
	class _TraceLog {
	public:
		_TraceLog(const std::wstring& str, UICallLogger* logger):
			m_msg(str), m_logger(logger)
		{
			if(m_logger!=nullptr)
				m_logger->recordCall(m_msg + L"--- Enter \n",true);
		}

		~_TraceLog(void) {
			if (m_logger != nullptr)
				m_logger->recordCall(m_msg + L"--- Leave \n",false);
		}
	protected:
		std::wstring m_msg;//same function name for Enter/Leave
		UICallLogger* m_logger;
	};

	UICallLogger(void) {};
	virtual ~UICallLogger(void) {};
	static UICallLogger* createLogger(void);
	virtual void recordCall(const std::wstring& msg,bool bEntry) = 0;
	virtual bool getRecordStr(std::wstring &str) = 0;
	virtual bool checkCallLoggerDog(void) = 0;
};
