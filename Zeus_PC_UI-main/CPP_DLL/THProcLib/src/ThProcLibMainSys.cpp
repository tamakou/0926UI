...
class CCallLoggerProc : public CThreadProc< CThProcMainSys>
{
public:
	CCallLoggerProc(CThProcMainSys* pMain) :
		CThreadProc< CThProcMainSys>(pMain, 0, 0)
		, m_pMain(pMain)
	{
...
CThProcMainSys::CThProcMainSys()
{
	...
	m_enable_dll_called_log = false;
}
CThProcMainSys::~CThProcMainSys()
{
	//#221_added_dll_called_log
	if (m_enable_dll_called_log) {
		if (m_thread_cmd_dog.m_proc_hdr != nullptr) {
			m_thread_cmd_dog.m_proc_hdr->cancelProc();
			if (m_thread_cmd_dog.m_theExe != nullptr) {
				m_thread_cmd_dog.m_theExe->wait(1000);
			}
		}
	}
	clearRenders();
...
//#127_shared_Haiya_Juukaku
	...
	//#221_added_dll_called_log
	//read from config
	m_enable_dll_called_log = config.m_DllCalledTrace != 0;
	if (m_enable_dll_called_log) {
		LogMessage(L" setup CCallLoggerProc thread \n");
		m_thread_cmd_dog.m_proc_hdr.reset(new CCallLoggerProc(this));
		m_thread_cmd_dog.m_theExe.reset(new CMultiThExe(1, m_thread_cmd_dog.m_proc_hdr.get()));
...
//#191_module_Version
static
std::wstring _getModuleVersion(const std::wstring& fileName)
{
	...
	if (pFileInfo) {
		DWORD verMS = pFileInfo->dwFileVersionMS;
		DWORD verLS = pFileInfo->dwFileVersionLS;
...
static 
std::wstring _get_path_myself(std::wstring &file_exe)
{
	std::wstring str;
	...
	fs::path exePath(path);            // exe のフルパス
	fs::path exeDir = exePath.parent_path(); // exe があるフォルダ

	file_exe = exePath.wstring();
	str = exeDir.wstring();
...
int CThProcMainSys::th_lib_get_env(ThProcLib_Env* env)
{
	...
	//#191_module_Version
	//m_SerialNumber
	std::wstring SN_str = config.m_SerialNumber;
	//ThMedServer.exe
	std::wstring img_serv_file = std::wstring(config.m_ImageServer_home) + L"bin/ThMedServer.exe";
	std::wstring img_serv_version = _getModuleVersion(img_serv_file);
	//ThDcm2Mesh.exe
	std::wstring ThDcm2Mesh_file = std::wstring(config.m_ThDcm2Mesh_folder) + L"bin/ThDcm2Mesh.exe";
	std::wstring ThDcm2Mesh_version = _getModuleVersion(ThDcm2Mesh_file);
	//
	std::wstring my_file_exe;
	std::wstring my_home_path = _get_path_myself(my_file_exe);
	//AppUI.exe
	std::wstring AppUI_file = my_home_path + L"/AppUI.exe";
	std::wstring AppUI_version = _getModuleVersion(AppUI_file);
	//ThProcLib.dll
	std::wstring ThProcLib_file = my_home_path + L"/ThProcLib.dll";
	std::wstring ThProcLib_version = _getModuleVersion(ThProcLib_file);
	//ThPCCoreLib.dll
	std::wstring ThPCCoreLib_file = my_home_path + L"/ThPCCoreLib.dll";
	std::wstring ThPCCoreLib_version = _getModuleVersion(ThPCCoreLib_file);
	//ThDcmMgrLib.dll
	std::wstring ThDcmMgrLib_file = my_home_path + L"/ThDcmMgrLib.dll";
	std::wstring ThDcmMgrLib_version = _getModuleVersion(ThDcmMgrLib_file);

	std::wstring module_ver;
//	module_ver = module_ver + std::wstring(L"製造番号: ") + SN_str + L"\n";
	module_ver = module_ver + std::wstring(L"AppUI: Ver") + AppUI_version + L"\n";
	module_ver = module_ver + std::wstring(L"ThProcLib: Ver") + ThProcLib_version + L"\n";
	module_ver = module_ver + std::wstring(L"ThPCCoreLib: Ver") + ThPCCoreLib_version + L"\n";
	module_ver = module_ver + std::wstring(L"ThDcmMgrLib: Ver") + ThDcmMgrLib_version + L"\n";
	module_ver = module_ver + std::wstring(L"ThDcm2Mesh: Ver") + ThDcm2Mesh_version + L"\n";
	module_ver = module_ver + std::wstring(L"ImageServer: Ver") + img_serv_version + L"\n";
	LogMessage(L"SN: %s \n", SN_str.c_str());
	LogMessage(L"=== Module Ver. === \n %s \n", module_ver.c_str());
	module_ver = std::wstring(L"製造番号: ") + SN_str + L"\n" + module_ver ;
...
#else
	_copy_wstr(env->ModelVers, module_ver.c_str());
#endif
...
int CThProcMainSys::exe_cmd_AiSegmentator(const ThProcLib_Cmd* cmd, ThProcLib_CmdOutput* cmd_ret)
{
	...
	switch (pos_in) {
	case ThLibPositionalCT_Supine:	    //仰臥位
		seg_pos = ThPCCoreLib::ThLibPositionalCT_Supine;
		pos_name = L"Supine";
		break;
	case ThLibPositionalCT_Left_Decubitus:   //1	    //左側臥位
		seg_pos = ThPCCoreLib::ThLibPositionalCT_Left_Decubitus;
		pos_name = L"Left Decubitus";
		break;
	case ThLibPositionalCT_Right_Decubitus:  //2     //右側臥位
...
void CThProcMainSys::checkCallLoggerDog(void)
{
	if (app_checkCallLoggerDog()) {
		
		std::wstring str;
		bool bCompleted = app_getAllLoggerStr(str);
		if (!bCompleted) {
			LogMessage(L"*** DLL calling not completed***\n");
		}
		LogMessage(L"%s\n", str.c_str());
	}
}