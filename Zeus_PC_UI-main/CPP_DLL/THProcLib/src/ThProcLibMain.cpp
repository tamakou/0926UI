...
int CThProcMain::exe_cmd_async_cancel(const ThProcLib_Cmd* cmd)
{
	...
	if (m_thread_cmd_async.m_proc_hdr!=nullptr) {//#205_Cancel_import_data_added_cancel_flag
		m_thread_cmd_async.m_proc_hdr->cancelProc();
		if (m_thread_cmd_async.m_theExe != nullptr) {
			m_thread_cmd_async.m_theExe->wait(1000);
		}
	}
	return TH_LIB_EXIT_SUCCESS;
}
...
class UICallLoggerImp : public UICallLogger
{
public:
	struct CallRec {
		std::wstring m_str;
	...
	UICallLoggerImp(int max = 128) {
		m_calling = true;
		m_queue_max = max; 
		m_called_tickcount = GetTickCount();
	};
	~UICallLoggerImp(void) {};

	bool getRecordStr(std::wstring& str_out) override
	{
		...
		int entry_on_count = 0;
		int entry_off_count = 0;
		while (!copy.empty()) {
			auto a = copy.front();
			if (a.m_entry){
				entry_on_count++;
			...
			str_out = str_out + a.m_str ;
			copy.pop();  
		}
		bCompleted = entry_on_count == entry_off_count;
		return bCompleted;
	}

	void recordCall(const std::wstring& msg, bool bEntry) override
	{
		std::lock_guard<std::mutex> lock(m_mtx);//auto lock/unlock
		m_calling = true;
		m_called_tickcount = GetTickCount();
		if (m_q.size() >= m_queue_max) {
			for (int i = 0; i < 10; i++) {
				m_q.pop();
			}
		}
		{
			wchar_t  time_str[128];
			{
				...
				int min = when.tm_min;
				int sec = when.tm_sec;
				swprintf(time_str, 128, L"%2d:%02d:%02d: ",
					hour, min, sec);
			}
			m_q.push({ std::wstring(time_str) + msg,bEntry });
		}
		
	};
	bool checkCallLoggerDog(void) override
	{
		if (!m_calling) return false;

		DWORD cur_time = GetTickCount();
		float diff_time = cur_time - m_called_tickcount;
		diff_time = (diff_time / 1000.0) ;
		bool rep_dog = diff_time > 30.0f;
		if (rep_dog) {
			m_calling = false;//stop it until next record;
			m_called_tickcount = cur_time;
		}
		return rep_dog;
	}
protected:
	bool m_calling;
	DWORD m_called_tickcount;
	int m_queue_max;
	std::mutex m_mtx;
	std::queue<CallRec> m_q;
};

UICallLogger* UICallLogger::createLogger(void)
{
	UICallLoggerImp* ret = new UICallLoggerImp();
	return ret;
}